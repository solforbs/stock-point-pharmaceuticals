    ...
    <div className="p-4 sm:p-6 space-y-4">
        ...
        {(['RECEIVABLE', 'PAYABLE', 'EXPIRY'] as const).map((key) => {
          ...
          return (
            <div key={key} className="ui-card p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-[11.5px] font-semibold text-slate-500">{categoryLabel[key]}</p>
                <p className="text-[19px] font-bold text-slate-800 tabular">{stats?.total ?? 0}</p>
                {!!stats?.critical && <p className="text-[11px] font-bold text-rose-600">{stats.critical} critical</p>}
              </div>
            </div>
          )
        })}
      ...
      <div className="ui-card overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex flex-wrap items-center gap-2">
          ...
          <label className="flex items-center gap-2 text-[12px] text-slate-600 font-semibold">
            ...
            {rows.map((alert) => (
              <tr key={alert.id} className="border-t border-slate-100 hover:bg-slate-50/70">
                <td className="px-4 py-2.5">
                  <Link to={alert.link ?? '#'} className="font-bold text-slate-800 hover:text-blue-700">{alert.title}</Link>
                  {alert.detail && <p className="text-[11.5px] text-slate-500 mt-0.5">{alert.detail}</p>}
                </td>
                <td className="px-4 py-2.5">
                  <span className={`inline-flex px-2 py-0.5 rounded-lg border text-[10.5px] font-bold ${severityStyle[alert.severity]}`}>
                ...
                <td className="px-4 py-2.5 tabular whitespace-nowrap">
                  ...
                  {alert.days_to_due !== null && (
                    <span className={`block text-[10.5px] font-semibold ${alert.days_to_due < 0 ? 'text-rose-600' : 'text-slate-500'}`}>
                ...
                <td className="px-4 py-2.5 text-right">
                  {alert.acknowledged_at ? (
                    <span className="text-[11px] text-slate-400 font-semibold">Seen</span>
                  ) : (
                    <button type="button" onClick={() => acknowledge.mutate(alert.id)} className="text-[11.5px] font-bold text-blue-600 hover:underline cursor-pointer">
                      Mark seen
                    </button>
                  )}
                </td>
