      ...
      <div className="space-y-4">
        <div id="tour-sync-connectivity" className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <section className="ui-card p-4 border-l-4" style={{ borderLeftColor: 'var(--status-slate)' }}>
            <div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-slate-900">Offline queue</h2><StatusBadge status="0 pending" tone="slate" label="0 pending" /></div>
            <p className="text-xs text-slate-500 mt-1">{s?.offline_queue.note ?? 'Offline selling is not built yet: every sale posts straight to the server, so nothing waits on a device.'}</p>
          </section>
        </div>
