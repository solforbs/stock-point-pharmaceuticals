...
export default function CreditControlPage() {
  ...
  const columns: Column<Customer>[] = [
    { key: 'name', header: 'Customer', render: (c) => <><div className="font-semibold">{c.name}</div><div className="text-[10.5px] text-[var(--text-muted)]">{c.code} · tier {c.tier?.code ?? '—'}</div></>, sortValue: (c) => c.name },
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_380px]">
        ...
        <Card title={selected ? selected.name : 'Select a customer'}>
          <div className="p-4 space-y-3">
            {!selected ? (
              <p className="text-[12px] text-[var(--text-muted)]">Pick a customer to change the limit or place a hold.</p>
            ) : (
              <>
                ...
                <label className="flex items-center gap-2 text-[12.5px]"><input type="checkbox" checked={onHold} disabled={!canOverride} onChange={(e) => setOnHold(e.target.checked)} /> Place on credit hold (blocks credit sales)</label>
