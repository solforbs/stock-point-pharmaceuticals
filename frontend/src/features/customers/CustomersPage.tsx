...
import { DescriptionList, DrawerFooter, Field, FormSection, Input, PrimaryAction, Select } from '../../components/ui/primitives'
...
export default function CustomersPage() {
  ...
  const columns: Column<Customer>[] = [
    ...
    { key: 'name', header: 'Name', render: (c) => <span className="font-bold text-slate-900">{c.name}</span>, sortValue: (c) => c.name },
    { key: 'type', header: 'Type', render: (c) => titleCase(c.customer_type) },
    { key: 'tier', header: 'Tier', render: (c) => <span className="font-bold text-blue-600">{c.tier?.code ?? '—'}</span> },
...
function CustomerDetailDrawer({ id, onClose }: { id: string | null; onClose: () => void }) {
  ...
  return (
    <Drawer
      open={!!id}
      onClose={onClose}
      title={c ? c.name : 'Customer Details'}
