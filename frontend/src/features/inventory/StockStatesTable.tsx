...
export function StockStatesTable({
  ...
  const columns: Column<StockStateRow>[] = [
      ...
      ? [
          {
            ...
            render: (r: StockStateRow) => (
              <div className="py-0.5">
                ...
                <div className="text-xs text-slate-500 font-mono mt-0.5">{r.product_code}</div>
              </div>
            ),
      ...
      : []),
    {
      ...
      header: 'Store',
      render: (r) => (
        <span className="inline-flex px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 font-semibold text-xs">
          {r.store_code}
        </span>
      ),
      ...
      render: (r) => {
        ...
        return (
          <span className="inline-flex items-center gap-1.5">
            <QtyCell value={r.free_to_sell} className={`font-bold ${!dIsPos(r.free_to_sell) ? 'text-rose-700' : ''}`} />
    ...
    { key: 'quarantined', header: 'Quarantined', align: 'right', render: (r) => <QtyCell value={r.quarantined} className={dIsPos(r.quarantined) ? 'text-purple-700' : ''} />, sortValue: (r) => Number(r.quarantined) },
    { key: 'expired', header: 'Expired', align: 'right', render: (r) => <QtyCell value={r.expired} className={dIsPos(r.expired) ? 'text-rose-700' : ''} />, sortValue: (r) => Number(r.expired) },
    { key: 'recalled', header: 'Recalled', align: 'right', render: (r) => <QtyCell value={r.recalled} className={dIsPos(r.recalled) ? 'text-purple-700' : ''} />, sortValue: (r) => Number(r.recalled) },
    { key: 'in_transit', header: 'In transit', align: 'right', render: (r) => <QtyCell value={r.in_transit} className={dIsPos(r.in_transit) ? 'text-blue-700' : ''} />, sortValue: (r) => Number(r.in_transit) },
